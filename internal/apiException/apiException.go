...
var (
	...
	UsernameError      = NewError(http.StatusInternalServerError, 200502, "用户名长度必须为12位")
	...
	//查看回复
	GetPostError = NewError(http.StatusInternalServerError, 200518, "用户没有提出反馈")
	GetResponseError = NewError(http.StatusInternalServerError, 200518, "查看回复失败")

	//做出评价
	TrashPost             = NewError(http.StatusInternalServerError, 200519, "反馈被标记为垃圾信息")
	...
	GetPostResponseTimeError = NewError(http.StatusInternalServerError, 200526, "获取回复时间失败")
	UpdatePostResponseTimeError = NewError(http.StatusInternalServerError, 200527, "更新回复时间失败")
